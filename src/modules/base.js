...
class BaseModule extends ModuleBase
{
    constructor()
    {
        ...
        this.add_command({
            match: message => {
                if(!message.content.startsWith("list my roles"))
                    return null;
                    
                return [];
            },
            sample: "!list my roles",
        ...
        this.add_command({
            ...
            description: "Lists all the timezones <not working>",
            permission: null,
            global: false,
        ...
        this.add_command({
        	match: message => {
        		if(!message.content.startsWith("8") &&
        		   !message.content.startsWith("ball") &&
        		   !message.content.startsWith("question") &&
        		   !message.content.startsWith("8ball") &&
         ...
         execute: this.handle_other
        });
        this.add_command({
        	match: message => {
        		if(!message.content.startsWith("roll") &&
        		   !message.content.startsWith("numberr"))
        			return null;

        	return[];
        	},
        sample: "!roll",
        description: "roll random number (0 - 100)",
        permission: null,
        global: false,

        execute: this.handle_roll
        });
      ...
      handle_other(message)
    {
       if(message)
        {
            if(message)
                return this.bot.respond(message, responses.get("OTHER").format({author: message.author.id}));
    }
      ...
      handle_roll(message)
    {
       if(message)
        {
            if(message)
                return this.bot.respond(message, responses.get("NUMBERS").format({author: message.author.id}));
    }
